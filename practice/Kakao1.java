...
public class Kakao1 {
	...
	public static String solution(String new_id) {
		...
		//2단계 new_id에서 알파벳 소문자, 숫자, 빼기(-), 밑줄(_), 마침표(.)를 제외한 모든 문자를 제거합니다.
		ArrayList<Integer> removeIdxList = new ArrayList<>();
        ...
        ArrayList<String> divStringList = new ArrayList<>();
        int prev = -1;
        for(int idx : removeIdxList) {
        	divStringList.add(new_id.substring(prev+1, idx));
        	//System.out.println(new_id.substring(prev+1, idx));
        	prev = idx;
        }
        divStringList.add(new_id.substring(prev+1));
        //System.out.println(new_id.substring(prev+1));
        for(String s : divStringList) {
        	answer += s;
        }
        //System.out.println("2단계 후 : " + answer);
        //3단계 new_id에서 마침표(.)가 2번 이상 연속된 부분을 하나의 마침표(.)로 치환합니다.
        prev = 0;
        HashMap<Integer, Integer> replaceIdxMap = new HashMap<>();
        for(int i=1; i<answer.length(); i++) {
        	if(answer.charAt(prev) == '.' && answer.charAt(i) == '.') {
        		if(replaceIdxMap.keySet().contains(prev)) {
        			int value = replaceIdxMap.get(prev);
        			replaceIdxMap.remove(prev);
        			replaceIdxMap.put(i, value);
        		}
        		else replaceIdxMap.put(i, prev);
        	}
        	prev = i;
        }
        String answer3 = "";
        prev = 0;
        for(int key : replaceIdxMap.keySet()) {
        	answer3 += answer.substring(prev, replaceIdxMap.get(key));
        	prev = key;
        }
        answer3 += answer.substring(prev); 
        //System.out.println("3단계 후 : " + answer3);
        answer = answer3;
		//4단계 new_id에서 마침표(.)가 처음이나 끝에 위치한다면 제거합니다.
        if(answer.length() > 1) {
        	if(answer.charAt(0) == '.')
        ...
        if(answer.length() <= 2) {
        	...
        	//System.out.println("7단계 후 : " + answer);
        }
